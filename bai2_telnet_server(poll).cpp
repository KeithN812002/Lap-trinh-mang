...
typedef struct {
    ...
    TelnetState state;
    char        username[128];
    char        recvBuf[BUFSIZE]; // Trang bi buffer de telnet gui tung ky tu vao day
    int         recvLen;
} TelnetClient;
...
int main()
{
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        clients[i].sock = -1;
        clients[i].recvLen = 0;
        memset(clients[i].recvBuf, 0, BUFSIZE);
    }
    ...
    while (1) {
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (clients[i].recvLen + bytesRecv < BUFSIZE) {
                ...
                clients[i].recvLen = 0; // chong tran buffer
            }

            // Kiem tra da go Enter chua
            char* newlinePos;
            while ((newlinePos = strchr(clients[i].recvBuf, '\n')) != NULL) {
                *newlinePos = '\0';
                ...
                // Nguoi dung da bam enter, bay gio xu ly input thoi!
